...
export function AppHeader({ step }: { step?: 'profile' | 'matches' | 'prepare' | 'xray' }) {
  ...
  return (
    <>
      ...
      {step && <nav className="journey" aria-label="Application preparation progress"><ol>{steps.map((item, index) => <li className={index <= activeIndex ? 'is-active' : ''} aria-current={index === activeIndex ? 'step' : undefined} key={item.id}><span className="journey-index" aria-hidden="true">{index + 1}</span><span className="journey-label">{item.label}</span></li>)}</ol></nav>}
    </>
  );
}
