...
export function ReadyView({ id }: { id: string }) {
  ...
  return (
    <main><AppHeader step="xray" /><section className="ready-page"><Link className="back-link" href={`/xray/${id}`}>← Application X-Ray</Link>
      ...
      <div className="handoff-grid"><section><p className="eyebrow">Guardian has checked</p><ul className="checked-list"><li><span>✓</span>Your information compared with prototype rules</li><li><span>✓</span>Required demo evidence prepared</li><li><span>✓</span>Known document issue reviewed by you; records remain unverified</li><li><span>✓</span>No known preventable blocker remains</li></ul></section><section><p className="eyebrow">Still outside Guardian’s control</p><ul className="unknown-list">{scholarship.unknowns.map((unknown) => <li key={unknown}><span>?</span>{unknown}</li>)}</ul></section></div>
