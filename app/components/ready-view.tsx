...
export function ReadyView({ id }: { id: string }) {
  ...
  if (!reviewStateHydrated) return <main><AppHeader step="xray" /><section className="ready-page"><div className="review-loading" role="status">Loading your saved demo review…</div></section></main>;
  const report = evaluateApplication(scholarship, profile, evidenceDocuments, reviewStates[id]?.resolvedFindingIds ?? []);
  if (report.readiness !== 'READY_TO_APPLY') {
    return <main><AppHeader step="xray" /><section className="ready-page"><Link className="back-link" href={`/xray/${id}`}>← Application X-Ray</Link><div className="not-ready-handoff"><span aria-hidden="true">!</span><p className="eyebrow">Not ready yet</p><h1>A preventable issue still needs review.</h1><p>Guardian derives readiness from unresolved findings. It cannot be bypassed by opening this screen directly.</p><ul>{report.unresolvedPreventable.map((finding) => <li key={finding.id}>{finding.title}: {finding.recommendedAction}</li>)}</ul><Link className="button button-primary" href={`/xray/${id}`}>Return to X-Ray</Link></div></section></main>;
  }
  const officialSource = scholarship.sources[0];
  return (
    <main><AppHeader step="xray" /><section className="ready-page"><Link className="back-link" href={`/xray/${id}`}>← Application X-Ray</Link>
      ...
      <div className="handoff-grid"><section><p className="eyebrow">Guardian assessment</p><ul className="checked-list"><li><span>✓</span>Your information compared with prototype rules</li><li><span>✓</span>Required demo evidence prepared</li><li className="reviewed-list-item"><span>↺</span><div><strong>Reviewed by you—not verified</strong><small>Known document issue reviewed; underlying records remain unchanged and unverified.</small></div></li><li><span>✓</span>No known preventable blocker remains</li></ul></section><section><p className="eyebrow">Still outside Guardian’s control</p><ul className="unknown-list">{scholarship.unknowns.map((unknown) => <li key={unknown}><span>?</span>{unknown}</li>)}</ul></section></div>
